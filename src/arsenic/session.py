...
class RequestHelpers:
    async def _request(self, *, url: str, method: str, data=None, raw=False, timeout=None):
        status, data = await self.connection.request(url=url, method=method, data=data, timeout=timeout)
...
class Session(RequestHelpers):
    ...
    async def get(self, url: str, timeout=None):
        await self._request(url="/url", method="POST", data={"url": self.bind + url}, timeout=timeout)
