...
async def impl(event_loop, request):
    if sys.platform == 'win32':
        if isinstance(event_loop, SelectorEventLoop) and request.param is AsyncioSubprocessImpl:
            raise pytest.skip("Asyncio Subprocess doesn't work on win32 with selector event loop")
    yield request.param()
...
async def test_start_stop_process(impl, unused_tcp_port):
    proc = await impl.start_process([sys.executable, __file__, str(unused_tcp_port)], sys.stdout)
    for _ in range(5):
        try:
