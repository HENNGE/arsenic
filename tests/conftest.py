...
def local_session_factory(name: str,
                          driver: str,
                          service: Type[services.Service],
                          browser: Type[browsers.Browser],
                          browser_opts: Optional[Dict[str, Any]]=None) -> Callable[[str], AsyncContextManager[Session]]:
...
def bsl_context():
    if 'BROWSERSTACK_LOCAL_IDENTIFIER' not in os.environ:
        raise pytest.skip('Browserstack API key set, but no local identifier configured (BROWSERSTACK_LOCAL_IDENTIFIER)')
    binary = find_binary('BrowserStackLocal')
    if not binary:
        raise pytest.skip('Browserstack API key set, but no BrowserStackLocal binary found')
    args = [
        binary,
        '--key',
